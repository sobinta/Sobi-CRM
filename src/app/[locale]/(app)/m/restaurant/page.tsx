...
export default async function RestaurantDashboard() {
  ...
  if (!data) notFound();
  if ("disabled" in data) {
    return <div className="p-8 text-sm text-ink-muted">The Restaurant module is not active for this workspace.</div>;
  }
  ...
  return (
    <div>
      <PageHeader title="Restaurant" description="Table reservations, guests, and seating." />
      <div className="mx-auto max-w-5xl space-y-6 px-6 py-6">
        <StatCards
          stats={[
            { label: "Upcoming", value: String(stats.upcoming), icon: CalendarDays, tone: "info" },
            { label: "Today", value: String(stats.today), icon: UtensilsCrossed, tone: "brand" },
            { label: "Completed", value: String(stats.completed), icon: CheckCircle2, tone: "positive" },
            { label: "Tables/menus", value: String(stats.services), icon: Users, tone: "neutral" },
          ]}
        />
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <CalendarDays className="h-4 w-4 text-brand" /> Upcoming reservations
            </CardTitle>
          </CardHeader>
          <CardContent>
            {upcoming.length === 0 ? (
                  ...
                  <li key={a.id} className="flex items-center justify-between gap-3 py-2.5">
                    <div className="min-w-0">
                      ...
                      <p className="truncate text-xs text-ink-muted">
                        {a.partySize ? `Party of ${a.partySize}` : "—"}
                      </p>
                    </div>
