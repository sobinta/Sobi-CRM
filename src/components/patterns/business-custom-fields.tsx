...
export function BusinessCustomFields({ entityKey, onChange }: { entityKey: string; onChange: (values: Record<string, unknown>) => void }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      if (demo) {
        try {
          const raw = localStorage.getItem(`sobi:demo-form:${user.activeTenantId}:${entityKey}`);
          if (raw) {
            const form = JSON.parse(raw) as FormDefinition;
            const definitions = (form.fieldDefinitions ?? []).filter((field) => !field.archived);
            ...
            if (active) {
              setResult({
                canCustomize: true,
                definition: {
                  meta: { key: entityKey, nameSingular: entityKey, namePlural: entityKey, source: "builtin", module: "custom", titleField: definitions[0]?.key ?? "id", fields: definitions },
                  form: customForm,
                },
              });
            }
          ...
          // Fall back to the persisted server definition.
        }
      }
      const next = await loadBusinessCustomFormAction(entityKey);
      if (active) setResult(next as typeof result);
    }
    void load();
    return () => { active = false; };
  }, [demo, entityKey, user.activeTenantId]);
