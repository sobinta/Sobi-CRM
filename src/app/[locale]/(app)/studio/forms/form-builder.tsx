...
export function FormBuilder({ meta, entityLabel, initial, versions }: { meta: EntityMetadata; entityLabel: string; initial: FormDefinition; versions: VersionRow[] }) {
  ...
  useEffect(() => {
    if (feedback || JSON.stringify(form) === JSON.stringify(initial)) return;
    const warn = (event: BeforeUnloadEvent) => { event.preventDefault(); };
    window.addEventListener("beforeunload", warn);
    return () => window.removeEventListener("beforeunload", warn);
  }, [feedback, form, initial]);
        ...
        {form.sections.map((section, sectionIndex) => (
          <article key={section.id} className="dashboard-glow-card overflow-hidden rounded-2xl border border-line bg-surface-raised shadow-raised">
            <header className="flex items-center gap-2 border-b border-line bg-surface-sunken/55 px-4 py-3">
              <input name={`section-title-${section.id}`} autoComplete="off" value={section.title ? labelFor(section.title, appLocale) : ""} onChange={(event) => update({ ...form, sections: form.sections.map((item) => item.id === section.id ? { ...item, title: typeof item.title === "object" ? { ...item.title, [appLocale]: event.target.value } : event.target.value } : item) })} aria-label={t("sectionTitle")} placeholder={t("sectionTitle")} className="min-w-0 flex-1 rounded-sm bg-transparent text-sm font-semibold text-ink outline-none focus-visible:outline-2 focus-visible:outline-focus-ring" />
            ...
            <div className="grid gap-3 p-4 sm:grid-cols-2">
              {!section.fields.length && <button type="button" onClick={() => setFieldOpen(true)} className="col-span-full min-h-28 rounded-xl border border-dashed border-line-strong text-sm text-ink-faint outline-none hover:border-brand hover:bg-brand-subtle hover:text-brand focus-visible:outline-2 focus-visible:outline-focus-ring">{t("emptySection")}</button>}
...
function IconButton({ label, children, onClick, danger, active }: { label: string; children: React.ReactElement<{ className?: string }>; onClick: () => void; danger?: boolean; active?: boolean }) { return <button type="button" aria-label={label} title={label} onClick={onClick} className={cn("inline-flex h-8 w-8 items-center justify-center rounded-lg text-ink-faint outline-none hover:bg-surface-sunken hover:text-ink focus-visible:outline-2 focus-visible:outline-focus-ring", danger && "hover:bg-danger-subtle hover:text-danger", active && "bg-brand-subtle text-brand")}>{<span aria-hidden="true" className="[&>svg]:h-3.5 [&>svg]:w-3.5">{children}</span>}</button>; }
...
function FieldDialog({ open, onOpenChange, existingKeys, onCreate }: { open: boolean; onOpenChange: (open: boolean) => void; existingKeys: Set<string>; onCreate: (field: FieldDefinition) => void }) {
  ...
  return <Dialog open={open} onOpenChange={(next) => { onOpenChange(next); if (!next) reset(); }}><DialogContent><DialogHeader><DialogTitle>{t("newFieldTitle")}</DialogTitle></DialogHeader><DialogBody className="grid gap-4 sm:grid-cols-2"><div><Label htmlFor="field-key" required>{t("fieldKey")}</Label><Input id="field-key" name="field-key" autoComplete="off" dir="ltr" value={key} onChange={(event) => setKey(event.target.value)} placeholder="customer_segment" /><p className="mt-1 text-[11px] text-ink-faint">{t("fieldKeyHint")}</p></div><div><Label htmlFor="field-type" required>{t("fieldType")}</Label><NativeSelect id="field-type" name="field-type" autoComplete="off" value={type} onChange={(event) => setType(event.target.value as FieldType)}>{FIELD_TYPES.map((value) => <option key={value} value={value}>{t(`types.${value}`)}</option>)}</NativeSelect></div>
    ...
    <div className="sm:col-span-2"><Label htmlFor="help-text">{t("helpText")}</Label><Input id="help-text" name="help-text" autoComplete="off" value={helpText} onChange={(event) => setHelpText(event.target.value)} /></div>{["select", "multiselect"].includes(type) && <div className="sm:col-span-2"><Label htmlFor="field-options" required>{t("options")}</Label><textarea id="field-options" name="field-options" autoComplete="off" value={options} onChange={(event) => setOptions(event.target.value)} rows={4} className="w-full rounded-md border border-line bg-surface-raised px-3 py-2 text-sm text-ink outline-none focus-visible:border-brand focus-visible:ring-2 focus-visible:ring-brand/15" placeholder={t("optionsHint")} /></div>}
